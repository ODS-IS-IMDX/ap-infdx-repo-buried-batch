...
@Component
@StepScope
public class RegistGenericErrorTasklet implements Tasklet {
    ...
    public RegistGenericErrorTasklet(ICommonApiManager commonApiService) {
     ...
     * <p>
     * 普通例外の処理を行う．<br>
     * 普通例外の内容と共に、設備データ出力タスク管理テーブルにステータスをエラーで更新する．
     * </p>
     * 
    ...
    public RepeatStatus execute(StepContribution stepContribution, ChunkContext context) {
        ...
        final PutOutputTasksRequestDto putTaskGenericErrorDto = PutOutputTasksRequestDto.builder()
                .taskId(taskId)
                .taskStatus(BatchCommonConstant.TASK_STATUS_ERROR)
                .errorDetail(errorMsg)
                .build();
        
        commonApiService.callPutOutputTasks(putTaskGenericErrorDto);
